...
class GradientBiasMonitor:
    ...
    (all_gather_object) and writes dense-matrix npz chunks on rank 0.
    ...
    ─── {step_tag}_hidden.npz ───────────────────────────────────────────────
      act_q              uint8   (S, A, R, B, L, T)         quantized activation norms
      act_scale          float32 (S, A, R, B, L)            per-sample scale
      act_min            float32 (S, A, R, B, L)            per-sample min
      act_outlier_idx    int32   (S, A, R, B, L, K_h)       positions in [0, T)
      act_outlier_val    float32 (S, A, R, B, L, K_h)
    ...
    ─── INDEX RECOVERY ──────────────────────────────────────────────────────
    For hidden array `X[s, a, r, b, l, t]`:
      global_step = global_steps[s]   accum_idx = a   rank = r
      sample_idx  = b                 layer_idx = l
      layer_type  = layer_type_legend[layer_types[l]]
    ...
    def __init__(self, orig_model, logs_dir, rank=0, world_size=1,
                 steps_per_file=2, outlier_pct=0.01, debug=False):
        ...
        self._global_step = 0
        self._accum_idx = 0
        self._buffer = []           # local records, drained by flush()
        self._fwd_cache = {}        # (kind, layer_idx, [qkv], accum_idx) -> dict
        self._flush_buffer = []     # rank-0 only: cross-step accumulator
        self._flush_step_start = None
        self._hooks = []
    ...
    def _setup_hooks(self):
        # --- hidden-state hooks on every discovered block ---
        for i, block in enumerate(self._hidden_modules):
            layer_type = self._layer_types[i]

            def make_fwd_hidden(layer_idx, layer_type):
                def fwd_hook(module, inp, output):
                    if not module.training:
                        return
                    # print(f"[fwd]\t{'hidden':<6}\tL{layer_idx:02d}\t{layer_type}")
                    ...
                    B, T, C = hidden.shape
                    # Compute norm in fp32 on-device, cast to fp16 for CPU storage.
                    norms = hidden.detach().float().norm(dim=-1).to(torch.float16).cpu()  # (B, T)
                    self._fwd_cache[("hidden", layer_idx, self._accum_idx)] = {
                        "gs": self._global_step, "ac": self._accum_idx,
                        "layer_type": layer_type,
                        "seq_len": T, "n_elements": C,
                        "act_norms": norms,  # (B, T) fp16 cpu — full batch, sample split deferred to flush
                    }
                return fwd_hook
            ...
            def make_bwd_hidden(layer_idx):
                def bwd_hook(module, grad_input, grad_output):
                    if not module.training:
                        return
                    # print(f"[bwd]\t{'hidden':<6}\tL{layer_idx:02d}\t{self._layer_types[layer_idx]}")
                    key = ("hidden", layer_idx, self._accum_idx)
                    cached = self._fwd_cache.pop(key, None)
                    if cached is None:
                        return
                    g = grad_output[0]
                    if g is None or g.dim() != 3:
                        return
                    grad_norms = g.detach().float().norm(dim=-1).to(torch.float16).cpu()  # (B, T)
                    # One buffer entry per (layer, accum); per-sample split happens at flush.
                    self._buffer.append({
                        "type": "hidden",
                        "rank": self.rank,
                        "global_step": cached["gs"],
                        "accum_idx": cached["ac"],
                        "layer_idx": layer_idx,
                        "layer_type": cached["layer_type"],
                        "seq_len": cached["seq_len"],
                        "n_elements": cached["n_elements"],
                        "act_norms": cached["act_norms"],  # (B, T) fp16 cpu tensor
                        "grad_norms": grad_norms,          # (B, T) fp16 cpu tensor
                    })
                return bwd_hook

            self._hooks.append(block.register_forward_hook(make_fwd_hidden(i, layer_type)))
        ...
        for spec in self._attn_specs:
            ...
            for qkv_name, proj in spec["proj"].items():
                ...
                def make_fwd_qkv(layer_idx, qkv, n_heads, head_dim, layer_type):
                    def fwd_hook(module, inp, output):
                        if not module.training:
                            return
                        # print(f"[fwd]\t{'attn':<6}\tL{layer_idx:02d}\t{layer_type}\tqkv={qkv}")
                        # output: (B, T, n_heads * head_dim)
                        B, T, _ = output.shape
                        act = output.detach().float().view(B, T, n_heads, head_dim)
                        per_norms = act.norm(dim=-1).to(torch.float16).cpu()  # (B, T, n_heads)
                        self._fwd_cache[("qkv", layer_idx, qkv, self._accum_idx)] = {
                            "gs": self._global_step, "ac": self._accum_idx,
                            "layer_type": layer_type,
                            "seq_len": T, "n_heads": n_heads, "head_dim": head_dim,
                            "act_norms": per_norms,  # (B, T, n_heads) fp16 cpu
                        }
                    return fwd_hook
                ...
                def make_bwd_qkv(layer_idx, qkv, n_heads, head_dim):
                    def bwd_hook(module, grad_input, grad_output):
                        if not module.training:
                            return
                        # print(f"[bwd]\t{'attn':<6}\tL{layer_idx:02d}\t{self._layer_types[layer_idx]}\tqkv={qkv}")
                        key = ("qkv", layer_idx, qkv, self._accum_idx)
                        cached = self._fwd_cache.pop(key, None)
                        if cached is None:
                            return
                        g = grad_output[0]
                        ...
                        gn = g_r.norm(dim=-1).to(torch.float16).cpu()  # (B, T, n_heads)
                        self._buffer.append({
                            "type": "attn_qkv",
                            "rank": self.rank,
                            "global_step": cached["gs"],
                            ...
                            "n_heads": n_heads,
                            "head_dim": head_dim,
                            "act_norms": cached["act_norms"],  # (B, T, n_heads) fp16 cpu
                            "grad_norms": gn,                  # (B, T, n_heads) fp16 cpu
                        })
                    return bwd_hook

                self._hooks.append(proj.register_forward_hook(
                    make_fwd_qkv(i, qkv_name, n_heads, head_dim, layer_type)))
                self._hooks.append(proj.register_full_backward_hook(
                    make_bwd_qkv(i, qkv_name, n_heads, head_dim)))

    # ---------------- flush (write parquet) ----------------
    def _gather_local_buffer(self):
        """DDP: all_gather_object → return merged list on rank 0, [] elsewhere.
        Single rank: just return local buffer. Always clears local buffer."""
        local = self._buffer
        self._buffer = []
    ...
    def flush(self, global_step=None, force=False):
        merged = self._gather_local_buffer()
        if self.rank != 0:
            return
        if merged:
            self._flush_buffer.extend(merged)
        if not self._flush_buffer:
            return

        all_steps = sorted(set(r["global_step"] for r in self._flush_buffer))
        if self._flush_step_start is None:
            self._flush_step_start = all_steps[0]

        if force:
            flush_end = all_steps[-1]
        elif all_steps[-1] - self._flush_step_start + 1 < self._steps_per_file:
            return
        else:
            flush_end = self._flush_step_start + self._steps_per_file - 1

        recs = [r for r in self._flush_buffer if r["global_step"] <= flush_end]
        self._flush_buffer = [r for r in self._flush_buffer if r["global_step"] > flush_end]
        if not recs:
            self._flush_step_start = flush_end + 1
            return

        actual_steps = sorted(set(r["global_step"] for r in recs))
        step_tag = f"step_{actual_steps[0]}-{actual_steps[-1]}"
        out_dir = os.path.join(self._logs_dir, "norms")

        hidden_recs = [r for r in recs if r["type"] == "hidden"]
        attn_recs   = [r for r in recs if r["type"] == "attn_qkv"]

        S = len(actual_steps)
        step_to_s = {gs: i for i, gs in enumerate(actual_steps)}
        A = max((r["accum_idx"] for r in recs), default=-1) + 1
        R = self.world_size
        L = self._n_layer
        T = recs[0]["seq_len"]
        ...
        global_steps_arr = np.array(actual_steps, dtype=np.int32)

        # infer B from any record
        B = int(recs[0]["act_norms"].shape[0])

        # -------- HIDDEN --------
        if hidden_recs:
            expected = S * A * R * L
            if len(hidden_recs) != expected:
                raise RuntimeError(
                    f"hidden records: got {len(hidden_recs)}, expected {expected} "
                    f"(S={S} A={A} R={R} L={L})")

            # Allocate dense fp32 buffers and scatter records into them.
            act = np.zeros((S, A, R, B, L, T), dtype=np.float32)
            grd = np.zeros((S, A, R, B, L, T), dtype=np.float32)
            for r in hidden_recs:
                s = step_to_s[r["global_step"]]
                a = r["accum_idx"]; rk = r["rank"]; l = r["layer_idx"]
                # r["act_norms"]: torch fp16 (B, T). Place into [s, a, rk, :, l, :].
                act[s, a, rk, :, l, :] = r["act_norms"].numpy().astype(np.float32)
                grd[s, a, rk, :, l, :] = r["grad_norms"].numpy().astype(np.float32)
            ...
            np.savez_compressed(
                ...
                layer_type_legend=layer_type_legend,
                n_elements=np.int32(hidden_recs[0]["n_elements"]),
                seq_len=np.int32(T),
        ...
        if attn_recs:
            expected = S * A * R * L * 3
            if len(attn_recs) != expected:
                raise RuntimeError(
                    f"attn records: got {len(attn_recs)}, expected {expected} "
                    f"(S={S} A={A} R={R} L={L} qkv=3)")

            q_act  = np.zeros((S, A, R, B, L, T, H_q),  dtype=np.float32)
            q_grd  = np.zeros((S, A, R, B, L, T, H_q),  dtype=np.float32)
            kv_act = np.zeros((S, A, R, B, L, 2, T, H_kv), dtype=np.float32)
            kv_grd = np.zeros((S, A, R, B, L, 2, T, H_kv), dtype=np.float32)
            kv_idx = {"k": 0, "v": 1}
            ...
            for r in attn_recs:
                ...
                a = r["accum_idx"]; rk = r["rank"]; l = r["layer_idx"]
                qkv = r["qkv"]
                an = r["act_norms"].numpy().astype(np.float32)  # (B, T, H)
                gn = r["grad_norms"].numpy().astype(np.float32)
                if qkv == "q":
                    q_act[s, a, rk, :, l, :, :] = an
                    q_grd[s, a, rk, :, l, :, :] = gn
                else:
                    kv = kv_idx[qkv]
                    kv_act[s, a, rk, :, l, kv, :, :] = an
                    kv_grd[s, a, rk, :, l, kv, :, :] = gn
...
parser.add_argument("--monitor-steps-per-file", type=int, default=5, help="flush one npz chunk per N global steps")

...
class GradientBiasMonitor:
        ...
        for i, block in enumerate(self._hidden_modules):
            ...
            def make_fwd_hidden(layer_idx):
                def fwd_hook(module, inp, output):
                    ...
                    # Compute norm in fp32 on-device, cast to fp16, copy into CPU slab slot.
                    norms = hidden.detach().float().norm(dim=-1).to(torch.float16)  # (B, T) on GPU
            ...
            def make_bwd_hidden(layer_idx):
                def bwd_hook(module, grad_input, grad_output):
                    ...
                    if g is None or g.dim() != 3:
                        return
                    gn = g.detach().float().norm(dim=-1).to(torch.float16)  # (B, T)
    ...
    def flush(self, global_step=None, force=False):
        ...
        hidden_act_g  = self._gather_slab(self._hidden_act[:n])     # (R, n, A, L, B, T) or None
        ...
        #   hidden: (R, n=S, A, L, B, T)       -> (S, A, R, B, L, T)
